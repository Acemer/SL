...
import ru.nordwest.nord.common.items.ItemMetalDrop;
import ru.nordwest.nord.common.items.ItemMetalIngot;
import ru.nordwest.nord.common.items.ItemMetalOre;
...
public class RegisterMetal {
	...
	*/
                Item item;
                Block block = null;
                for (final String metall : RegisterMetal.metall_list) {
                        block = new BlockMetal(Material.iron)
                                .setBlockName(metall + "_block")
                                .setCreativeTab(TabsNord.tabMetals)
                                .setBlockTextureName(metall + "_block").setHardness(3.0F)
                                .setHardness(5.0F);
                        block.setHarvestLevel("pickaxe", 2);
                        GameRegistry.registerBlock(block, ItemMetalBlock.class, metall
                                + "_block");
                        RegisterMetal.metall_block.put(metall, block);
                        ...
                        RegisterMetal.metall_ingot.put(metall, item);
                        for (int i = 0; i < 4; i++) {
                ...
                for (int i = 0; i < RegisterMetal.ores.size(); i++) {
                        final String name = RegisterMetal.ores.get(i).name;
                        if ((i % 16) == 0) {
                                block = new BlockMetalOre(Material.rock, i)
                                        .setCreativeTab(TabsNord.tabMetals).setResistance(5.0F).setBlockName("ore_" + i);
                                GameRegistry.registerBlock(block, ItemMetalOre.class, "ore1_"
                                        + i);
                        }
                        if (block != null) {
                                RegisterMetal.metall_ore.put(name, new ItemStack(block, 1,
                                        i % 16));
                                RegisterMetal.metall_drop.put(name, new ItemStack(drop, 1, i));
                                RegisterMetal.metall_powder.put(name, new ItemStack(powder, 1,
                                        i));
                                RegisterMetal.metall_clear_powder.put(name, new ItemStack(
                                        clear_powder, 1, i));
                        }
                }
        ...
        public static ItemStack getMetallBlock(final String name, final int quality, final int quantaty) {
                if (RegisterMetal.metall_block.containsKey(name)) {
                        return new ItemStack(RegisterMetal.metall_block.get(name), quantaty,
                                quality % 4);
                } else {
        ...
        public static ItemStack getMetallIngot(final String name, final int quality, final int quantity) {
                if (RegisterMetal.metall_ingot.containsKey(name)) {
                        return new ItemStack(RegisterMetal.metall_ingot.get(name), quantity,
                                quality % 4);
                } else {
        ...
        public static ItemStack getMetallPowder(final String name, final int quantaty) {
                if (RegisterMetal.metall_ingot.containsKey(name)) {
                        return new ItemStack(RegisterMetal.metall_block.get(name), quantaty, 5);
                } else {
        ...
        public static ItemStack getOre(final String name, final int quantaty) {
                if (RegisterMetal.metall_ore.containsKey(name)) {
                        ItemStack item = RegisterMetal.metall_ore.get(name);
        ...
        public static ItemStack getOreDrop(final String name, final int quantaty) {
                if (RegisterMetal.metall_drop.containsKey(name)) {
                        ItemStack item = RegisterMetal.metall_drop.get(name);
                        item.stackSize = quantaty;
                        return item;
                } else {
        ...
        public static ItemStack getOrePowder(final String name, final int quantaty) {
                if (RegisterMetal.metall_powder.containsKey(name)) {
                        ItemStack item = RegisterMetal.metall_powder.get(name);
                        item.stackSize = quantaty;
                        return item;
                } else {
        ...
        public static ItemStack getOreClearPowder(final String name, final int quantaty) {
                if (RegisterMetal.metall_clear_powder.containsKey(name)) {
                        ItemStack item = RegisterMetal.metall_clear_powder.get(name);
                        item.stackSize = quantaty;
                        return item;
                } else {
