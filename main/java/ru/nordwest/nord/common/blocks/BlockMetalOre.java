...
public class BlockMetalOre extends BlockMetadata {
        ...
        public BlockMetalOre(final Material par2Material, final int shift) {
                super(par2Material, 16);
                this.shift = shift;
                this.type = RegisterMetal.ore_list.length > (shift + 16)
                        ? 16
                        : RegisterMetal.ore_list.length % 16;
        }
        ...
        public void registerBlockIcons(final IIconRegister par1IconRegister) {
                ...
                for (int i = 0; i < this.type; i++) {
                        this.texture[i] = par1IconRegister.registerIcon(Nord.MODID
                                + ":ore/" + RegisterMetal.ore_list[this.shift + i]);
                }
        }
        ...
        public float getBlockHardness(World world, int x, int y, int z) {
                return RegisterMetal.ores.get(shift + world.getBlockMetadata(x, y, z)).hard;
        }

        @Override
        public Item getItemDropped(int metadata, Random rand, int fortune) {
                final String name = RegisterMetal.ores.get(0).name;
                return RegisterMetal.getOreDrop(name, 1).getItem();
        }
